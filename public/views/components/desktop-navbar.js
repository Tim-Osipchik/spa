...
import CreatePlayListButton from './create-playlist-button.js'
...
const DesktopNavbar = () => {
    ...
    const createPlaylist = async () => {
        playlistAPI.createPlaylist(user.uid, 'New playlist')
        ...
        const view = `
            ...
            <ul class="links">
                <li class="nav-link-item">
                    <a href="/#/" class="nav-link main-nav-link link-subtitle">
                        ...
                        <span class="nav-link-text text-cut-one-line">
                            Главная
                        </span>
                    </a>
                </li>
                <li class="nav-link-item">
                    <a href="/#/search" class="nav-link main-nav-link link-subtitle">
                        <div class="icon home-icon">
                            <svg viewBox="0 0 512 512" width="24" height="24" xmlns="http://www.w3.org/2000/svg">
                ...
                <li class="nav-link-item">
                    <a href="/#/playlists" class="nav-link main-nav-link link-subtitle">
                        <div class="icon home-icon">
                            <svg viewBox="0 0 512 512" width="24" height="24" xmlns="http://www.w3.org/2000/svg">
            ...
            <div class="albums-group">
                ${await CreatePlayListButton(createPlaylist).render()}
                <hr class="divider"/>
    ...
    const renderPlaylists = async (user) => {
        const playlists = await playlistAPI.getUserPlaylists(user.uid)
        const renderedItems = playlists.val.map(playlist => playlistItem(playlist.info)).join('\n ')
        const playlistsSection = document.getElementById('user-playlists')
        
        playlistsSection.innerHTML = renderedItems
    }
    ...
    const afterRender = async () => {
        ...
        if (user) {
            await renderPlaylists(user)

            const button = document.getElementById('createPlaylist-navButton')
    ...
    const playlistItem = ({id, name}) => {
        return `
            <li class="playlist-link-wrapper">
                <a href="/#/playlist/${id}" class="px-24 nav-link playlist-link text-cut-one-line">
                    ${name}
                </a>
            </li>
        `
    ...
    UserStore.addListener(async (user) => await afterRender())
